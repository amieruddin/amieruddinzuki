...
import get_ip
import configparser
import basler_config as BaslerConfig
import sys
...
try:
    standby=cv2.imread('asset/standby.jpg')
    if standby is None:
...
def get_time_period():
    ...
    if curr_time >= datetime.time(17, 0) or curr_time < datetime.time(7, 0):  # Start night at 17:00, end at 07:00
        return "night"
    else:
        return "day"
    ...
    try:
        ...
        while True :
            try:
                ...
                print(color.RED + "\nAnother app is using camera.\nPlease close Pylon Viewer or any app related.\n\n" + color.END ,e )
                sys.exit()
            
        #camera=pylon.InstantCamera(pylon.TlFactory.GetInstance().CreateFirstDevice())
        camera.Open()
        print("start camera setting")
        BaslerConfig.setConfig(camera)
